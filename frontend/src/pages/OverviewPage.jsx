    ...
    <div ref={rootRef} className="overview-page bg-background text-on-surface font-body-md overflow-hidden h-screen flex flex-col">
      <header className="fixed top-0 left-0 w-full z-[60] flex items-center justify-between px-3 sm:px-gutter h-16 bg-surface-container-low border-b border-outline-variant">
        <div className="flex items-center gap-2 sm:gap-8">
          <button onClick={toggle} className="mobile-menu-btn p-2 -ml-2 hover:bg-surface-variant/50 rounded-full transition-colors">
            <span className="material-symbols-outlined text-on-surface">menu</span>
          </button>
          <h1 className="font-display-md text-xl sm:text-display-md font-bold text-primary tracking-tight">
            ThermaCity
          </h1>
          ...
          <div className="hidden lg:flex items-center gap-3 px-4 border-l border-outline-variant">
            <span className="font-data-sm text-data-sm text-on-surface-variant uppercase tracking-widest">Temporal Range</span>
            <span className="font-data-sm text-data-sm text-primary">May 2024 - Jun 2024</span>
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="flex items-center gap-2 bg-on-primary-container/10 border border-primary/20 px-2 sm:px-3 py-1 rounded-full">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-primary"></span>
            </span>
            <span className="font-data-sm text-data-sm font-bold text-primary">LIVE</span>
          </div>
          <div className="flex items-center gap-1 sm:gap-2">
            <button className="p-2 hover:bg-surface-variant/50 transition-colors rounded-full">
      ...
      <div className="flex flex-1 pt-16 h-full overflow-hidden">
        ...
        <main className="relative flex-1 bg-surface-container-lowest overflow-hidden">
          ...
          <div className="absolute top-6 right-6 hidden sm:flex flex-col items-center gap-3 z-[1000]">
            <div className="bg-surface-container-low/90 backdrop-blur border border-outline-variant p-3 rounded-xl flex flex-col items-center">
              <span className="font-data-sm text-[10px] mb-2 text-on-surface-variant">60°C</span>
              <div className="w-3 h-48 thermal-gradient rounded-full"></div>
              <span className="font-data-sm text-[10px] mt-2 text-on-surface-variant">20°C</span>
            </div>
          </div>
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[90%] sm:w-[80%] max-w-2xl bg-surface-container-low/95 backdrop-blur border border-outline-variant p-3 sm:p-4 rounded-xl shadow-2xl z-[1000]">
            <div className="flex items-center justify-between mb-3 px-2">
              <span className="font-data-sm text-[10px] sm:text-data-sm text-on-surface-variant">
                {formatTime(new Date(new Date().setHours(8, 0, 0, 0)).toISOString(), settings.time_format)}
              </span>
              <span className="font-data-sm text-[10px] sm:text-data-sm text-primary font-bold">
        ...
        <aside className="hidden xl:flex w-80 z-40 flex-col bg-surface-container-lowest border-l border-outline-variant overflow-y-auto custom-scrollbar shrink-0">
          <div className="p-6 border-b border-outline-variant">
            ...
            <div className="flex flex-col items-center py-4 bg-surface-container-low rounded-xl border border-outline-variant/30 mb-6">
              ...
              <span className="font-data-sm text-data-sm text-error/80 uppercase tracking-widest">Surface Temp</span>
            </div>
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                <span className="font-body-sm text-on-surface-variant">Heat Risk Index</span>
                <span className="font-data-lg text-error">{selectedZone?.heat_risk_index ?? "--"}/10</span>
              </div>
          ...
          <div className="p-6 border-b border-outline-variant bg-surface-container-low/30">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-body-md font-bold text-on-surface">AI/ML Insights</h3>
            </div>
            {recs?.top_heat_drivers ? (
              <>
                ...
                <div className="space-y-3">
                  <h4 className="font-data-sm text-[10px] text-on-surface-variant uppercase tracking-widest mb-2">SHAP Feature Importance</h4>
                    ...
                    {recs.top_heat_drivers.slice(0, 5).map((d, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-3">
                          <span className="w-20 font-data-sm text-[10px] truncate">{d.feature.replace(/_/g, " ")}</span>
          ...
          <div className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-body-md font-bold text-on-surface">Intervention Simulator</h3>
            </div>
            <div className="space-y-4 mb-6">
              <div>
                <label className="block font-data-sm text-[10px] text-on-surface-variant uppercase mb-2">Select Strategy</label>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <label className="font-data-sm text-[10px] text-on-surface-variant uppercase">Implementation Coverage</label>
                  <span className="font-data-sm text-primary">{simCoverage}%</span>
                </div>
              ...
              <button onClick={async () => { if (!selectedZone) return; setSimRunning(true); try { const r = await simulateIntervention(selectedZone.zone_id, simType, simCoverage); setSimResult(r); } catch (e) { setSimResult({ error: e.message }); } finally { setSimRunning(false); } }} disabled={simRunning || !selectedZone} className="w-full py-3 bg-primary text-on-primary font-bold text-body-sm rounded hover:brightness-110 transition-all flex items-center justify-center gap-2 disabled:opacity-40">
                <span className="material-symbols-outlined text-sm">{simRunning ? "hourglass_top" : "play_arrow"}</span>
                {simRunning ? "SIMULATING..." : "RUN SIMULATION"}
              </button>
            </div>
            <div className="bg-primary/10 border border-primary/30 p-4 rounded-lg">
              <h4 className="font-data-sm text-[10px] text-primary uppercase mb-3 font-bold">Predicted Outcome</h4>
